...
test.describe('Admin Movies Management', () => {
  ...
  test('should login and navigate to admin movies', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/admin-movies-list.png', fullPage: true });
  ...
  test('should access movie edit page', async ({ page }) => {
    ...
    if (editButtonCount > 0) {
      ...
      expect(currentUrl).toMatch(/\/admin\/movies\/[a-f0-9-]+$/);
      
      // Take screenshot of the edit page
      await page.screenshot({ path: 'tests/screenshots/admin-movie-edit.png', fullPage: true });
      ...
      await page.screenshot({ path: 'tests/screenshots/admin-movies-no-edit.png', fullPage: true });
  ...
  test('should handle API errors gracefully', async ({ page }) => {
    ...
    // Take screenshot
    await page.screenshot({ path: 'tests/screenshots/admin-movie-error.png', fullPage: true });
    
    // Should show error message or redirect
    const hasError = await page.locator('text=映画が見つかりません').isVisible();
