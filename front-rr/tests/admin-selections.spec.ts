...
test.describe('Admin Movie Selections Management', () => {
  ...
  test('should display movie selections page with daily, weekly, monthly cards', async ({ page }) => {
    // Login first
    ...
    await expect(page.locator('a[href="/admin/movies"]')).toContainText('← 映画一覧に戻る');
    
    // Check for three selection cards
    await expect(page.locator('[data-testid="daily-selection"]')).toBeVisible();
    await expect(page.locator('[data-testid="weekly-selection"]')).toBeVisible();
    await expect(page.locator('[data-testid="monthly-selection"]')).toBeVisible();
    
    // Check card headers
    await expect(page.locator('[data-testid="daily-selection"] h2')).toContainText('今日の映画');
    await expect(page.locator('[data-testid="weekly-selection"] h2')).toContainText('今週の映画');
    await expect(page.locator('[data-testid="monthly-selection"] h2')).toContainText('今月の映画');
    
    // Check for Override Selection buttons
    await expect(page.locator('button:has-text("Override Selection")')).toHaveCount(3);
    
    // Take screenshot for verification
    await page.screenshot({ path: 'tests/screenshots/admin-selections.png', fullPage: true });
  });

  test('should open override modal when Override Selection button is clicked', async ({ page }) => {
    ...
    // Check modal content
    await expect(page.locator('[data-testid="override-modal"] h3')).toContainText('映画選択をオーバーライド');
    
    // Check tabs
    await expect(page.locator('[data-testid="search-tab"]')).toContainText('映画を検索');
    await expect(page.locator('[data-testid="random-tab"]')).toContainText('ランダム選択');
  ...
  test('should handle random movie selection', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="random-movie-result"]')).toBeVisible();
  });
  ...
  test('should handle API errors gracefully', async ({ page }) => {
    ...
    await page.screenshot({ path: 'tests/screenshots/admin-selections-error.png', fullPage: true });
  });
});