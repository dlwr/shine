...
export function meta({ data }: Route.MetaArgs): Route.MetaDescriptor[] {
  ...
  const title = movieDetail.translations?.find((t) => t.languageCode === 'ja')?.content || '映画詳細';
  
  return [
    { title: `${title} (${movieDetail.movie.year}) | SHINE` },
    { name: "description", content: `${title} (${movieDetail.movie.year}年) の詳細情報。受賞歴、ポスター、その他の情報をご覧いただけます。` }
  ];
}
...
export default function MovieDetail({ loaderData }: Route.ComponentProps) {
  if ('error' in loaderData) {
    const title = loaderData.status === 404 ? '映画が見つかりません' : 'エラーが発生しました';
  ...
  const title = movieDetail.translations?.find((t) => t.languageCode === 'ja')?.content || 'タイトル不明';
  const posterUrl = movieDetail.posterUrls?.find((p) => p.isPrimary)?.url || movieDetail.posterUrls?.[0]?.url;
  
  const winningNominations = movieDetail.nominations?.filter((n) => n.isWinner) || [];
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {(winningNominations.length > 0 || nominees.length > 0) && (
              <section>
                <h2 className="text-xl font-semibold text-gray-800 mb-4">受賞・ノミネート</h2>
                <div className="space-y-3">
                  ...
                  {nominees.map((nomination, index) => (
                    ...
                    >
                      🎬 {nomination.ceremony.name} {nomination.ceremony.year} ノミネート
                      <div className="text-xs mt-1">{nomination.category.name}</div>
                    </div>
                  ))}
                </div>
