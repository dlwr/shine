      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <section className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">今日の映画</h2>
            {movieSelections.daily && (
              <MovieCard 
                movie={movieSelections.daily}
              />
            )}
          </section>
          ...
          {/* 今週の映画 */}
          <section className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">今週の映画</h2>
            {movieSelections.weekly && (
              <MovieCard 
                movie={movieSelections.weekly}
              />
            )}
          </section>
          ...
          {/* 今月の映画 */}
          <section className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">今月の映画</h2>
            {movieSelections.monthly && (
              <MovieCard 
                movie={movieSelections.monthly}
              />
            )}
          </section>
        </div>
...
function MovieCard({ movie }: { movie: MovieData }) {
  const title = movie.translations?.find((t: { languageCode: string }) => t.languageCode === 'ja')?.content || 'タイトル不明';
  const posterUrl = movie.posterUrls?.[0]?.url;
  const winningNominations = movie.nominations?.filter((n: { isWinner: boolean }) => n.isWinner) || [];

  return (
    <div className="space-y-4">
      <a href={`/movies/${movie.movieUid}`} className="block hover:opacity-80 transition-opacity">
        {posterUrl && (
          <img 
            src={posterUrl} 
            alt={title}
      ...
      <div className="space-y-1">
        ...
        {winningNominations.length > 0 && (
          <div className="space-y-1">
            {winningNominations.map((nomination: { ceremony: { name: string; year: number } }, index: number) => (
              <span 
                key={index}
