...
const createValidationMiddleware = <T>(schema: z.ZodSchema<T>) => {
  return async (c: Context, next: Next) => {
    try {
      ...
      c.set("validatedData", validatedData as T);
      await next();
    } catch (error) {
...
export const articleLinkSchema = z.object({
  ...
  title: z.string().min(1, "Title is required").max(200, "Title too long").transform(sanitizeText),
...
export const translationSchema = z.object({
  ...
  content: z.string().min(1, "Content is required").max(500, "Content too long").transform(sanitizeText)
});
...
export const validateArticleLink = createValidationMiddleware(articleLinkSchema);
export const validateTranslation = createValidationMiddleware(translationSchema);
...
export const validateReselection = createValidationMiddleware(reselectionSchema);