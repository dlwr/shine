...
export const getCacheKeyForSelection = (type: string, date: string, locale: string): string => {
  return `selections:${type}:${date}:${locale}:v1`;
};

export const getCacheKeyForMovie = (movieId: string, includeDetails = false): string => {
...
export const getCacheKeyForUrlTitle = (url: string): string => {
  const urlHash = btoa(url).replaceAll(/[+/=]/g, '').slice(0, 16);
...
export const shouldCacheSearch = (
  ...
  const commonQueries = [
    'アカデミー', 'oscar', 'cannes', 'カンヌ', '日本アカデミー',
    'winner', '受賞', 'ノミネート', 'nominated'
  ];
...
export const checkETag = (request: { header: (name: string) => string | undefined }, etag: string): boolean => {
