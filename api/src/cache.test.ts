...
import { EdgeCache, getCacheKeyForSelection, getCacheKeyForMovie, getCacheTTL, createCachedResponse, createETag, checkETag, shouldCacheSearch } from "../src/utils/cache";
...
describe("Cache Utilities", () => {
  ...
  describe("Cache TTL Configuration", () => {
    ...
    it("should have longer TTL for less frequently changing data", () => {
      expect(getCacheTTL.movie.full).toBeGreaterThan(getCacheTTL.selections.daily);
      expect(getCacheTTL.selections.monthly).toBeGreaterThan(getCacheTTL.selections.weekly);
      expect(getCacheTTL.selections.weekly).toBeGreaterThan(getCacheTTL.selections.daily);
    });
  });
  ...
  describe("ETag Generation and Validation", () => {
    ...
    it("should generate different ETags for different data", () => {
      const data1 = { title: "Very Different Movie Title A", year: 2024, uid: "movie-1" };
      const data2 = { title: "Completely Different Movie Title B", year: 2025, uid: "movie-2" };
    ...
    it("should correctly validate ETags", () => {
      ...
      const mockRequestDifferent = {
        header: (name: string) => name === 'If-None-Match' ? '"different"' : undefined
      };
  ...
  describe("Cache Response Creation", () => {
    it("should create proper cached response", () => {
      ...
      expect(response.headers.get('Cache-Control')).toBe(`public, max-age=${ttl}, s-maxage=${ttl}`);
  ...
  describe("Cache Invalidation Patterns", () => {
    it("should support pattern-based cache deletion", async () => {
      ...
      await expect(cache.deleteByPattern("selections:")).resolves.toBeTypeOf("number");
      await expect(cache.deleteByPattern("movie:")).resolves.toBeTypeOf("number");
    });
  });
});
