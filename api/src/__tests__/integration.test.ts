...
describe('Integration Tests', () => {
  ...
  describe('Admin Operations Integration', () => {
    it('should integrate movie creation with all related data', async () => {
      ...
      // Create movie
      const movieResult = await mockDatabase
        .insert({} as never)
        .values({
          uid: 'new-movie-1',
          year: 2024,
      ...
      // Add translation
      const translationResult = await mockDatabase
        .insert({} as never)
        .values({
      ...
      // Add poster
      const posterResult = await mockDatabase
        .insert({} as never)
        .values({
  ...
  describe('Data Consistency Integration', () => {
    it('should maintain referential integrity across operations', async () => {
      ...
      const nominationResult = await mockDatabase
        .insert({} as never)
        .values({
          movieUid: _movieUid,
          categoryUid,
    ...
    it('should handle concurrent operations safely', async () => {
      ...
      const operations = [
        mockDatabase.update({} as never).set({ imdbId: 'tt1111111' }).where({} as never),
        mockDatabase.insert({} as never).values({ movieUid: _movieUid, url: 'poster1.jpg' }),
        mockDatabase.insert({} as never).values({ resourceUid: _movieUid, content: 'Title' }),
        mockDatabase.update({} as never).set({ year: 2024 }).where({} as never),
      ]
