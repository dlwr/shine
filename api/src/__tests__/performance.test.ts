      ...
      from: vi.fn(() => ({
        where: vi.fn(() => {
          ...
          return new Promise(resolve => {
            setTimeout(() => {
              resolve(Array.from({ length: 1000 }, (_, index) => ({
                uid: `movie-${index}`,
                year: 2000 + (index % 24),
        ...
        limit: vi.fn((count) => {
          return Promise.resolve(Array.from({ length: Math.min(count, 100) }, (_, index) => ({
            uid: `movie-${index}`,
            year: 2020 + index,
            originalLanguage: 'en',
          })))
        ...
        orderBy: vi.fn(() => {
          return Promise.resolve(Array.from({ length: 50 }, (_, index) => ({
            uid: `movie-${index}`,
            year: 2023 - index,
    ...
    it('should handle mixed read/write operations efficiently', async () => {
      ...
      const operations = [
        ...
        ...Array.from({ length: 20 }, () =>
          mockDatabase.select().from({} as never).limit(5)
        ),
        // Write operations
        ...
        // Update operations
        ...Array.from({ length: 5 }, () =>
          mockDatabase.update({} as never).set({ updated: true }).where({} as never)
        ),
      ]
    ...
    it('should handle request parsing at scale', () => {
      ...
      const requests = Array.from({ length: 100 }, (_, index) => ({
        ...
        body: JSON.stringify({
          ...
          description: `A test movie description that is quite long and contains various characters: ${index}`.repeat(5),
          year: 2000 + (index % 24),
