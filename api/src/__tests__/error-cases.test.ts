...
describe('Error Cases and Edge Cases', () => {
  ...
  describe('Input Sanitization Edge Cases', () => {
    ...
    it('should handle null input to sanitizeUrl', () => {
      expect(() => sanitizeUrl(undefined as unknown as string)).toThrow('Invalid URL')
    })
    ...
    it('should handle data: protocol URLs', () => {
      expect(() => sanitizeUrl('data:text/html,<script>alert("xss")</script>')).toThrow('Invalid URL')
    })
  })
  ...
  describe('Database Operation Edge Cases', () => {
    ...
    it('should handle database query timeout', async () => {
      ...
      const mockDatabase = {
        select: vi.fn(() => ({
          from: vi.fn(() => ({
            where: vi.fn(() => Promise.reject(new Error('Query timeout'))),
          })),
        })),
      }
      ...
      try {
        await database.select().from({} as never).where({} as never)
    ...
    it('should handle invalid database operations', async () => {
      ...
      const mockDatabase = {
        insert: vi.fn(() => ({
          values: vi.fn(() => Promise.reject(new Error('Invalid operation'))),
        })),
      }
