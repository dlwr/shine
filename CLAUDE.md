...
- **`nominations.personUid`**: 個人賞（監督賞・演技賞）は人物に紐づく。作品賞は `person_uid IS NULL` の部分ユニークインデックス、個人賞は人物を含む部分ユニークインデックスで一意性を担保しているので、作品側のクエリには `person_uid IS NULL` を付ける。作品賞の賞ページは `awardPageDefinitions`、個人賞の賞ページと `/people` のランキングは `personAwardDefinitions` で定義する（どちらも `award-definitions.ts`。`awards-service.ts` はロジックだけ）。`/years`・crossings・uncrowned は `awardPageNominations()` で作品賞に絞ること。映画祭のグランプリ・審査員賞は `subAward: true` で定義し、この集計からは除く（`findTopAwardPageDefinition`）
