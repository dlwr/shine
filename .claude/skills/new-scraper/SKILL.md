...
8. **dry-run**: データベースは `getScrapeDatabase(context)`（`common/dry-run.ts`）で取る。dry-run中に `insert`/`update`/`delete` を呼ぶと例外になるので、書き込み経路の分岐漏れがテストで露見する。`context` に `isDryRun: boolean` を持たせること
...
ローカルの一時DBで試せる。`TURSO_DATABASE_URL="file:/tmp/x.db"` を渡せば認証トークンは要らない（`assertDatabaseEnvironment` が `file:` を特別扱いする）。マイグレーションは `migrate()` を呼ぶ小さい `.mts` をパッケージ内に置いて `npx tsx` で流す（スクラッチパッド直下だとモジュール解決に失敗する）。
