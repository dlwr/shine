...
// Add nomination
adminNominationsRoutes.post('/movies/:movieId/nominations', authMiddleware, async c => {
  try {
...
// Update nomination
adminNominationsRoutes.put('/nominations/:nominationId', authMiddleware, async c => {
  try {
    const database = getDatabase(c.env);
...
// Delete nomination
adminNominationsRoutes.delete('/nominations/:nominationId', authMiddleware, async c => {
  try {
    const database = getDatabase(c.env);
    const nominationId = c.req.param('nominationId');
    if (!nominationId) {
