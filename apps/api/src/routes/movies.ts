...
moviesRoutes.get('/:id', async c => {
  try {
    ...
    if (cachedResponse) {
      ...
      return c.json(cachedResponse.data as Record<string, unknown>, 200, {'X-Cache-Status': 'HIT'});
    }
...
moviesRoutes.get('/:id/related', async c => {
  try {
    ...
    return createCachedResponse(result, getCacheTTL.movie.related, {'X-Cache-Status': 'MISS'});
  } catch (error) {
