...
export function createCommand(): Command {
  return new Command()
    ...
    .addHelpText(
      ...
      `
例:
  pnpm scrapers mainichi-film-concours --dry-run
  pnpm scrapers mainichi-film-concours --year 2025
`,
    )
    .action(
      async (options: {year?: number; dryRun: boolean; throttle: number}) => {
        ...
        const failed =
          stats.grandPrix.failed +
          stats.excellence.failed +
          stats.foreign.failed;
        if (failed > 0) {
          process.exitCode = 1;
        }
      },
