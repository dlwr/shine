 ...
 * @returns レスポンスオブジェクト
 */
async function handleSingleMovieScraping(movieId: string): Promise<Response> {
  try {
...
export default {
  ...
  async fetch(request: Request, environment: Environment): Promise<Response> {
    ...
    if (
      ...
      return handleBatchScraping(environment);
    }
    ...
    if (path.startsWith('/japanese-translations/movie/')) {
      ...
      return handleSingleMovieScraping(movieId);
    }
