...
describe('Database Basic Tests', () => {
  ...
  describe('SQLite Connection', () => {
    ...
    it('should query data from tables', () => {
      ...
      const insert = sqlite.prepare('INSERT INTO movies_test (uid, title, year) VALUES (?, ?, ?)')
      insert.run('test-1', 'Test Movie', 2024)

      const select = sqlite.prepare('SELECT * FROM movies_test WHERE uid = ?')
      const row = select.get('test-1') as { uid: string; title: string; year: number } | undefined
    ...
    it('should enforce unique constraints', () => {
      ...
      const insert = sqlite.prepare('INSERT INTO unique_test (email) VALUES (?)')
    ...
    it('should handle foreign key relationships', () => {
      ...
      const insertAuthor = sqlite.prepare('INSERT INTO authors (name) VALUES (?)')
      const authorResult = insertAuthor.run('Test Author')

      const insertBook = sqlite.prepare('INSERT INTO books (title, author_id) VALUES (?, ?)')
      ...
      `)
      const book = selectBook.get() as { title: string; author_name: string } | undefined
    ...
    it('should handle transactions', () => {
      ...
      const insert = sqlite.prepare('INSERT INTO transaction_test (value) VALUES (?)')
      ...
      const count = sqlite.prepare('SELECT COUNT(*) as count FROM transaction_test').get() as { count: number }
    ...
    it('should validate data types', () => {
      ...
      const row = select.get() as { id: number; text_field: string; integer_field: number; real_field: number }
